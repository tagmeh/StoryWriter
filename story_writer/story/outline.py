...
def generate_general_story_details(client: openai.Client, user_prompt) -> Path | None:
    ...
    general_story_data, elapsed = validated_stream_llm(client=client, messages=messages, model=model,
                                                       response_format=response_format, validation_model=GeneralData)
    ...
    with open(story_root / "story_data.yaml", mode="w+", encoding="utf-8") as f:
        yaml.dump(StoryData(general=general_story_data).model_dump(mode='json'), f, default_flow_style=False,
                  sort_keys=False)

    utils.log_step(story_root=story_root, messages=messages, file_name="expand_initial_prompt",
                   model=FIRST_PASS_GENERATION_MODEL,
