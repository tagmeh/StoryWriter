...
from config.story_settings import LLM_EMPTY_OUTPUT_RETRY_COUNT, LLM_INVALID_OUTPUT_RETRY_COUNT
...
def validated_stream_llm(client, messages, model, validation_model: Type[T], response_format: str | None = None,
                         max_retries: int = LLM_INVALID_OUTPUT_RETRY_COUNT
                         ) -> (Union[Type[T], list[Type[T]]], float):
    start = time.time()
    max_retries = max_retries
    for attempt in range(max_retries):
        content = stream_llm(client=client, messages=messages, model=model, response_format=response_format)
...
def stream_llm(client, messages, model, response_format: str | None, max_retries: int = LLM_EMPTY_OUTPUT_RETRY_COUNT):
    max_retries = max_retries
    retries = 0
